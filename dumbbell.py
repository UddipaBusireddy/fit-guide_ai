        ...
        while True:
            ...
            try:
                ...
                if 11 in idx_to_coordinates and 13 in idx_to_coordinates and 15 in idx_to_coordinates: 
                    ...
                    # self.correct_form = self.check_form(ang1)

                    # Calculate angle for left side
                    if ang1 < 70:
                        self.curlUp = True
                        
                    if ang1 > 130 and self.curlUp:
                        self.curlDown = True

                    if self.curlUp  and self.curlDown and self.correct_form:
                        self.count += 1
                        self.curlUp = False
                        self.curlDown = False
                ...
                if 12 in idx_to_coordinates and 14 in idx_to_coordinates and 16 in idx_to_coordinates:
                    ...
                    # Check if the form is correct
                    # self.correct_form = self.check_form(ang2)
            ...
            '''if not self.correct_form:
                cv2.putText(image, "Incorrect form!", (50, 100), 
                            fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                            fontScale=1.2, color=(0, 0, 255), thickness=5)
                self.count = 0'''

            cv2.putText(image, "Dumbbell Curls: " + str(self.count),
...
if __name__ == "__main__":
    dumbbell_curls = DumbbellCurls()
    dumbbell_curls.exercise(0)  # Use 0 for webcam or provide the path to the video file
